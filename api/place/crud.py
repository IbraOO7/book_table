...
from databases.models.place import Category
...
class CRUDBase(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    async def create_category(self, session: AsyncSession, obj_in: CreateSchemaType) -> ModelType:
        obj_in_data = dict(obj_in)
    ...
    async def multi_get_category(self, session: AsyncSession, *args, offset: int = 0, limit: int = 100, **kwargs) -> List[ModelType]:
        result = await session.execute(select(Category).filter(*args).filter_by(**kwargs).offset(offset).limit(limit))
        return result.scalars().all()
        
    async def update(self, session: AsyncSession, *args, 
    ...
    async def delete(self, session: AsyncSession, *args, db_obj: Optional[ModelType] = None,**kwargs) -> ModelType:
