...
from databases.models.order import Cost
...
class CRUDBase(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    async def create_order(self, session: AsyncSession, obj_in: CreateSchemaType) -> ModelType:
        obj_in_data = dict(obj_in)
        db_obj = Cost(**obj_in_data)
        session.add(db_obj)
        await session.commit()
        return db_obj
    ...
    async def call_order(self, session, db: AsyncSession):
